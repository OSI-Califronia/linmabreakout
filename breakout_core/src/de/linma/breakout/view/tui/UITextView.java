package de.linma.breakout.view.tui;
			...
			while (controller.getState() != GAME_STATE.KILLED && input.hasNextLine()) {
				...
				if (controller.getState() == GAME_STATE.RUNNING) {
					...
					if (strInput.equals(CHAR_MOVE_LEFT)) {
						controller.processGameInput(IGameController.PLAYER_INPUT.LEFT);
					} else if (strInput.equals(CHAR_MOVE_RIGHT)) {
						controller.processGameInput(IGameController.PLAYER_INPUT.RIGHT);
					} else if (strInput.equals(CHAR_PAUSE_GAME)) {
						controller.processGameInput(IGameController.PLAYER_INPUT.PAUSE);
					}
